...
class WalkerCharacter {
    ...
    // Persona
    var characterSystemPrompt: String?

    // Daily briefing
    var pendingBriefingDetail: String?
    ...
    private func openOnboardingPopover() {
        ...
        let welcome = """
        hey! we're boury and buddy — your lil dock agents.

        click either of us to open an AI chat. we'll walk around while you work and let you know when the AI is thinking.

        buddy gives you daily AI news + codebase updates. boury checks your email, calendar, and tasks every morning (type /connect in her terminal to link Google).
    ...
    func openPopover() {
        ...
        if session == nil {
            ...
            if let prompt = characterSystemPrompt {
                (newSession as? ClaudeSession)?.systemPrompt = prompt
            }
            session = newSession
        ...
        if let briefing = pendingBriefingDetail {
            pendingBriefingDetail = nil
            terminalView?.appendBriefing(briefing)
        }
