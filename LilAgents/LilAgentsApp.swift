...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func setupMenuBar() {
        ...
        let char1Item = NSMenuItem(title: "Boury", action: #selector(toggleChar1), keyEquivalent: "1")
        char1Item.state = .on
        menu.addItem(char1Item)

        let char2Item = NSMenuItem(title: "Buddy", action: #selector(toggleChar2), keyEquivalent: "2")
        char2Item.state = .on
        ...
        let integrationsItem = NSMenuItem(title: "Integrations", action: nil, keyEquivalent: "")
        let integrationsMenu = NSMenu()
        integrationsMenu.addItem(NSMenuItem(title: "Connect Boury to Google…", action: #selector(openGoogleConnect), keyEquivalent: ""))
        integrationsMenu.addItem(NSMenuItem(title: "Connect Buddy to GitHub…", action: #selector(openGitHubSetup), keyEquivalent: ""))
        integrationsItem.submenu = integrationsMenu
        menu.addItem(integrationsItem)

        // Briefing time submenu
        let briefingItem = NSMenuItem(title: "Briefing Time", action: nil, keyEquivalent: "")
        let briefingMenu = NSMenu()
        let hours = [(6, "6:00 AM"), (7, "7:00 AM"), (8, "8:00 AM"), (9, "9:00 AM"),
                     (10, "10:00 AM"), (11, "11:00 AM"), (12, "12:00 PM")]
        let currentHour = LilAgentsController.briefingHour
        for (hour, label) in hours {
            let item = NSMenuItem(title: label, action: #selector(setBriefingTime(_:)), keyEquivalent: "")
            item.tag = hour
            item.state = hour == currentHour ? .on : .off
            briefingMenu.addItem(item)
        }
        briefingItem.submenu = briefingMenu
    ...
    @objc func openGoogleConnect() {
        GoogleCredentialsWindow.show()
    }
    ...
    @objc func setBriefingTime(_ sender: NSMenuItem) {
        LilAgentsController.briefingHour = sender.tag
        if let menu = sender.menu {
            menu.items.forEach { $0.state = $0.tag == sender.tag ? .on : .off }
        }
        // Reset last-run dates so briefing re-fires at the new time today if applicable
        UserDefaults.standard.removeObject(forKey: "BuddyLastBriefingDate")
        UserDefaults.standard.removeObject(forKey: "BouryLastBriefingDate")
    }
